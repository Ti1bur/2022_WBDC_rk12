...
from single_stream.VIT import vit
...
def parse_args():
    ...
    parser.add_argument('--ViT_pretrained', type=str, default='../../opensource_models/ViT-B-16.pt')
    parser.add_argument('--output_path', type=str, default='/opt/ml/input/data/zip_feats/unlabeled_vit.zip')
    args = parser.parse_args()
    return args


def build_model(ViT_pretrained) -> torch.nn.Module:
    """ Load the pretrianed feature extractor (VIT here). """
    model = vit()
...
def main():
    args = parse_args()
    os.makedirs('/opt/ml/input/data/zip_feats/', exist_ok=True)
    model = build_model(args.ViT_pretrained)
    ...
    with torch.no_grad():
        ...
        for dataitem in tqdm(dataloader):
            ...
            feature = model(img.to(torch.float16))
    ...
    args.ann_path = '/opt/ml/input/data/annotations/labeled.json'
    args.zip_frame_dir = '/opt/ml/input/data/zip_frames/labeled/'
    args.output_path = '/opt/ml/input/data/zip_feats/labeled_vit.zip'
    model = build_model(args.ViT_pretrained)
    dataset = RawFrameDataset(args.ann_path, args.zip_frame_dir)
    # batch-size == 8 is fine for V100 GPU, please consider use smaller batch-size if OOM issue occurs.
    dataloader = DataLoader(dataset, batch_size=8, num_workers=24, shuffle=False, pin_memory=True, drop_last=False)
