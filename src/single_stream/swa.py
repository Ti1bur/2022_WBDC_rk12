...
import torch
import copy
from model import UniBert, MultiModal
...
def get_model_path_list(base_dir):
    ...
    for root, dirs, files in os.walk(base_dir):
        for _file in files:
            # if 'epoch_0' not in _file and 'epoch_1' not in _file and 'epoch_4' not in _file:
            model_lists.append(os.path.join(root, _file))
    return model_lists

def swa(model, model_dir, swa_start=12):
    ...
    with torch.no_grad():
        ...
        swa_model_path = os.path.join(swa_model_dir, 'single_model.bin')
        torch.save(swa_model.state_dict(), swa_model_path)
    return swa_model

if __name__ == '__main__':
    dir = "./save/vit-14-pretrain30-ema-pgd3-origin-dataset/"   # swa_dir = "E:/code\微信大数据挑战赛/weixin/save/SWA/
    args = parse_args()
